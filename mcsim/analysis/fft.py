...
def ift2(m: array, axes: tuple[int] = (-1, -2), plan=None) -> array:
    if isinstance(m, cp.ndarray) and _gpu_available:
...
def ift2_adj(m: array, axes: tuple[int] = (-1, -2), plan=None) -> array:
    if isinstance(m, cp.ndarray) and _gpu_available:
...
# ######################
def ft3(m: array, axes: tuple[int] = (-1, -2, -3), plan=None) -> array:
    """
...
def ift3(m: array, axes: tuple[int] = (-1, -2, -3), plan=None) -> array:
    if isinstance(m, cp.ndarray) and _gpu_available:
